...
export class AttrezzaturaGetallComponent {
  ...
  getDataFromResolver() {
    console.log("stampo quello che arriva");
      ...
      const action = [
        {
          ...
          type: "icon",
          callback: () => this.updateAttrezzatura(r.id),
        },
        {
          title: LABEL_CONSTANT.elimina,
          icon: ICON_CONSTANT.delete,
          type: "icon",
          callback: () => this.deleteAttrezzatura(r.id),
        },
      ];
      return {
        ...
        prezzo: r.prezzo,
        idImpianto: r.idImpianto ? r.idImpianto.nome : "-", // da effettuare modifica qui se necessario
        action: action,
      };
    });
  }
