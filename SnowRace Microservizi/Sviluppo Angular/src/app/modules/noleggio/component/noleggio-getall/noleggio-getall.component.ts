...
export class NoleggioGetallComponent {
  ...
  getMappedDataSource(toMap: any[]) {
    return toMap
      .filter(r => r.idnoleggio && r.username && r.idattrezzatura && r.startDate && r.endDate) 
      .map((r) => {
        const action = [
          {
        ...
        return {
          ...
          idattrezzatura: r.idattrezzatura.nome,
  ...
  //       idattrezzatura: r.idattrezzatura ? r.idattrezzatura : '-',
  //       data_inizio: r.startDate,
  //       data_fine: r.endDate,
  //       action: action,
  //     };
