...
export class FiltroPrenotazioneComponent {
  ...
  submitForm() {
    ...
    if (this.filter === true) {
      this.prenService.filteredDates(25, 0, valori.dataInizio, this.idUtente).subscribe({
        next: (res: any) => {
      ...
      this.prenService
        //fare una read nell(apostrofo)oninit dove fai una read dell(apostrofo)utente a partire dallo username e ti restituisce tutto il dto dove ti andrai a prendere l(apostrofo)id e lo passi come ultimo argomento nel metodo sotto
        .filteredPiste(25, 0, this.myForm.value.id, this.idUtente)
        .subscribe({
          next: (res: any) => {
            this.totalElements = res.totalElements;
            this.pageIndex = res.pageIndex;
  ...
  getMappedDataSource(toMap: any[]) {
    ...
    return toMap.map((r) => {
      ...
      //     DataFine: r.dataFine,
      //     nomePista: r.idPista.nome,
      //     nomeUtente: r.idUser.username,
      //     action: action,
      //   };
      // } else {
      //   if (
      //     !(r.idUser && this.loginService.getUtenteSessione().username === r.idUser.username)
      //   ) {
      //     return undefined;
      //   }
      return {
