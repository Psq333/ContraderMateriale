...
export class ImpiantiService {
  ...
  resolveListaImpiantiAdmin(): Observable<any> | undefined {
    ...
    return this.getListaImpiantiAdmin(25, 0, username).pipe(
      catchError((error) => {
        return of("No data");
      })
    );
  ...
  getListaImpianti(
    ...
    let url = `${this.baseUrl}impianto/getall?pageSize=${pageSize}&pageNumber=${pageNumber}`;
    args.forEach((x: any) => {
      if (x) {
  ...
  getListaImpiantiAdmin(
    ...
    let url = `${this.baseUrl}impianto/getAllPaginata?pageSize=${pageSize}&pageNumber=${pageNumber}&username=${username}`;
    args.forEach((x: any) => {
      if (x) {
        for (const [key, value] of Object.entries(x)) {
          url = url + `&${key}=${value}`;
  ...
  updateImpianto(
    ...
    return this.http.put<ImpiantoModelRequest>(
      this.baseUrl + "impianto/update",
      payload
    );
  }
