...
public class UserServlet extends HttpServlet {
		...
		if(mode != null) {
			switch (mode.toUpperCase()) {
				...
				case "READ":
					...
					AnagraficaDTO anag = anagService.read(user.getUsername());
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(mode != null) {
			...
			UserDTO user = new UserDTO (username,password, Usertype.valueOf(usertype));//todo
			switch (mode.toUpperCase()) {
				case "INSERT":
				...
				case "REGISTER":
					if(service.insert(user)) {
						...
						request.getRequestDispatcher("/anagrafica/anagraficaregistration.jsp").forward(request, response);
					} else {
						//throw new RuntimeException("Registrazione non andata a buon fine");
