...
public class AnagraficaDAO implements DAO<Anagrafica>{
    ...
    private final String QUERY_READ_ID = "SELECT * FROM anagrafica WHERE idutenti=?";
    ...
    private final String QUERY_DELETE_ID = "DELETE FROM anagrafica WHERE idutenti=?";
    ...
    public boolean insert(Anagrafica anagraficaToInsert) {
        ...
        try {
            ...
            preparedStatement.setDate(5, (java.sql.Date) anagraficaToInsert.getData_nascita());
            preparedStatement.setString(6, anagraficaToInsert.getUsername());
    ...
    public boolean update(Anagrafica anagraficaToUpdate) {
        ...
        if (anagraficaToUpdate.getUsername() == null || anagraficaToUpdate.getUsername().equals(""))
            return false;
        Anagrafica anagraficaRead = read(anagraficaToUpdate.getUsername());
        if (!anagraficaRead.equals(anagraficaToUpdate)) {
            try {
                ...
                preparedStatement.setString(6, anagraficaToUpdate.getUsername());
    ...
    public boolean delete(int id) {
        Connection connection = ConnectionSingleton.getInstance();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(QUERY_DELETE_ID);
            preparedStatement.setInt(1, id);
            int n = preparedStatement.executeUpdate();
            if (n != 0)
                return true;
