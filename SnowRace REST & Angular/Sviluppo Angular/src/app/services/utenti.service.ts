...
export class UtentiService {
  ...
  getAdmin(): Observable<any> {
    let url = `${this.baseUrl}user/getAllAdmin`;
    return this.http.get<any>(url);
  }

  getUtente(user: any): Observable<any> {
    let url = `${this.baseUrl}user/read?id=${user}`;
    return this.http.get<any>(url);
  }
  getUtenteByUsername(username: any): Observable<any> {
    let url = `${this.baseUrl}user/readusername?username=${username}`;
    return this.http.get<any>(url);
  }
...
getTuttiUtenti(): Observable<any>{
  ...
  delete(id: number):  Observable<any>{
    let url = `${this.baseUrl}user/deleteUser?id=${id}`;
    return this.http.delete<any>(url);
  }

  modifica(payload: any): Observable<any> {
    let url = `${this.baseUrl}user/update`;
    return this.http.put(url, payload);
  }

  insert(body: any): Observable<any> {
    let url = `${this.baseUrl}user/insert`;
