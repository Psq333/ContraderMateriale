...
export class GetallPrenotazioneComponent {
  ...
  listaPrenotazioni: any[] = []; // Inizializzazione di un array vuoto da riempire con dati di qualsiasi tipo (any)
      ...
      .subscribe({
        ...
        next: (res) => {
          ...
          if (res.content) {
            ...
            this.listaPrenotazioni = res.content; //aggiorna la lista con i dati della risposta
  ...
  getMappedDataSource(toMap: any[]) {
    ...
    return toMap.map((r) => {
      ...
      return {
        ...
        DataFine: r.dataFine,
        nomePista: r.pista ? r.pista.nome: '-',
        nomeUtente: r.user ? r.user.username : '-',
        action: action,
      };
    });
  }
