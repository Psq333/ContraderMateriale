...
export class AttrezzaturaInsertComponents {
  ...
  async submitForm() {
    try {
      ...
      const res = await this.impiantiService.readImpianto(formValue.idImpianto).toPromise();
      ...
      if (this.isModifica) {
        await this.attrezzaturaService.updateAttrezzatura(formValue).toPromise();
        console.log(formValue);
        window.location.reload();
      } else {
        await this.attrezzaturaService.insertAttrezzatura(formValue).toPromise();
        window.location.reload();
      }
    } catch (error) {
  ...
  setModifica(id: number) {
    ...
    this.attrezzaturaService.readAttrezzatura(id).subscribe({
      next: (res: any) => {
        this.attrezzaturaInsert = this.fb.group({
          idAttrezzatura: res.idAttrezzatura,
          nome: res.nome,
