...
export class InserisciImpiantoComponent {
  ...
  async submitForm() {
    let k = await this.getAdmin(this.inserimentoImpianto.value.amministratore).then(
      () => {
        this.inserimentoImpianto.value.amministratore = this.amm;
    if (this.loginService.isAdmin()) {
  ...
  inserimentoModificaAmministratore(){
    var observable;
    console.log("LATO SUPERADMIN");
    console.log(this.inserimentoImpianto.value);
    if(this.isModifica === true){
      this.isModifica = false;
        observable = this.impiantiService.updateImpianto(this.inserimentoImpianto.value);
    }
    else{
      observable = this.impiantiService.inserisciImpianto(this.inserimentoImpianto.value);
    }
    observable.subscribe({
      next: () => {
          window.location.reload();
